...
function Root({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="empty"
      className={cn(
        "gap-4 rounded-xl p-6 flex w-full min-w-0 flex-col items-center justify-center text-center text-balance border",
        className,
      )}
      {...props}
    />
...
function Title({ className, ...props }: React.ComponentProps<"div">) {
  return <h2 data-slot="empty-title" className={className} {...props} />;
}
